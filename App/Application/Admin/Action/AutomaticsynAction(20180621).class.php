...
class AutomaticsynAction extends Action{
    public function index(){
		...
		foreach ($data_access as $key=>$v){
			D('Access_record')->execute("INSERT INTO bd_access_record (uid,url,ref,kw,source,region,ip,browser,platform,add_time) VALUES ('".$v['uid']."','".$v['url']."','".$v['ref']."','".$v['kw']."','".$v['source']."','".$v['region']."','".$v['ip']."','".$v['browser']."','".$v['platform']."','".$v['add_time']."');"); 
			$last=$v['id'];
		}	
		$last=$last+1;
		//清理数据	
		...
		foreach ($data_copy as $key=>$v){
			D('Copy_record')->execute("INSERT INTO bd_copy_record (uid,url,ref,kw,source,region,ip,browser,platform,add_time,copy_content) VALUES ('".$v['uid']."','".$v['url']."','".$v['ref']."','".$v['kw']."','".$v['source']."','".$v['region']."','".$v['ip']."','".$v['browser']."','".$v['platform']."','".$v['add_time']."','".$v['copy_content']."');"); 
			$last=$v['id'];
		}	
		$last=$last+1;
		//清理数据	
		...
		foreach ($data_press as $key=>$v){
			D('Press_record')->execute("INSERT INTO bd_press_record (uid,url,ref,kw,source,region,ip,browser,platform,add_time,copy_content) VALUES ('".$v['uid']."','".$v['url']."','".$v['ref']."','".$v['kw']."','".$v['source']."','".$v['region']."','".$v['ip']."','".$v['browser']."','".$v['platform']."','".$v['add_time']."','".$v['copy_content']."');"); 
			$last=$v['id'];
		}	
		$last=$last+1;
		//清理数据	
